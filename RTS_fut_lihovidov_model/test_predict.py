"""
Тест куда пишутся предсказания. Для текущей строки или следующей.
"""
...
model_path = Path(r"test_predict.pth")
...
df_fut['CONTROL_PREDICTION'] = None  # Создаём колонку для контрольных прогнозов

with torch.no_grad():
    for i in range(len(df_fut) - window_size):
        # Берём окно из последних `window_size` свечей
        sequence = torch.tensor(
            df_fut['CANDLE_INT'].iloc[i:i + window_size].values, dtype=torch.long
        ).unsqueeze(0).to(device)
        ...
        # Сохраняем результат в соответствующую строку
        df_fut.loc[i + window_size, 'CONTROL_PREDICTION'] = 1 if pred > 0.5 else 0

        print(i)  # Контроль выполнения

# Удаление строк с NaN
df_fut.dropna(inplace=True)

# Преобразование колонки 'values' из float в int
