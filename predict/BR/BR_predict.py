...
def save_results(csv_file_path, results):
    ...
    if csv_file_path.exists():
        ...
        if df['TRADEDATE'].iloc[-1] in existing_data['TRADEDATE'].values:
            print(f"📌 BR. Записи с датой {df['TRADEDATE'].iloc[-1].date()} уже существуют в файле.")
        else:
            # Дописываем новый DataFrame в файл
            results.to_csv(csv_file_path, mode='a', header=False, index=False)
            print(f"📌 BR. Результаты добавлены в файл {csv_file_path}.")
    else:
        # Если файл не существует, создаем его и записываем данные
        results.to_csv(csv_file_path, index=False)
        print(f"📌 BR. Файл {csv_file_path} создан и данные записаны.")
...
if __name__ == '__main__':
    ...
    csv_file_path = Path(r'BR_results.csv')
    start_date = '2024-11-01'  # Начальная дата
    model_1_path = Path(fr"model\best_model_73.pth")
    model_2_path = Path(fr"model\best_model_88.pth")
