...
def prepare_data(df_fut):
    ...
    X_train_bal = torch.tensor(X_train_bal).reshape(num_samples, 1, INPUT_SIZE)  
    y_train_bal = torch.tensor(y_train_bal)

    X_val = torch.tensor(X_val).reshape(-1, 1, INPUT_SIZE)  
...
if __name__ == "__main__":
    set_seed(42)  # 🔥 Фиксируем случайные числа!
