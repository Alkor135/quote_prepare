...
class CustomLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true, candle_bodies, avg_body_10):
        # Прогнозируемые классы (0 или 1)
        y_pred_classes = (y_pred > 0.5).float()

        # Правильный прогноз (1) или неправильный (0)
        correct = (y_pred_classes == y_true).float()

        # Определяем, является ли тело свечи больше среднего
        large_body = (candle_bodies > avg_body_10).float()

        # Весовые коэффициенты:
        # - Правильный прогноз с большим телом: +2
        # - Правильный прогноз с малым телом: +1
        # - Неправильный прогноз с малым телом: +1
        # - Неправильный прогноз с большим телом: +2
        weights = correct * (1 + large_body) + (1 - correct) * (1 + large_body)

        # Ограничиваем минимальный вес, чтобы избежать нулевых значений
        weights = torch.clamp(weights, min=0.1)

        # Вычисляем базовую бинарную кросс-энтропию
        loss = nn.BCELoss(reduction="none")(y_pred, y_true)
