...
def balance_classes(X_candle, X_day, y):
    """
    Функция для балансировки классов методом Oversampling.
    """
    # Создаем DataFrame
    df_candle = pd.DataFrame(X_candle, columns=[f'CI_{i}' for i in range(1, 21)])
    df_day = pd.DataFrame(X_day, columns=[f'DAY_W_{i}' for i in range(0, 20)])
    df_train = pd.concat([df_candle, df_day], axis=1)
    ...
    X_day_bal = df_balanced[[f'DAY_W_{i}' for i in range(0, 20)]].values
...
def data_load(db_path, start_date, end_date):
    ...
    # Удаление колонок CANDLE_CODE, CANDLE_INT, DAY_W
    df_fut = df_fut.drop(columns=['CANDLE_CODE', 'CANDLE_INT', 'DAY_W'])
    ...
    for col in [f'DAY_W_{i}' for i in range(0, 20)]:
        df_fut[col] = pd.to_numeric(df_fut[col], errors='coerce').fillna(0).astype(np.int64)
