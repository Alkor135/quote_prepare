...
class CustomLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true, candle_bodies, avg_body_20):
        ...
        weights = torch.clamp(weights, min=0.1)  # Исключаем отрицательные значения весов

        # Отладка
        print(f"y_pred: {y_pred[:5].detach().cpu().numpy()}")
        print(f"y_true: {y_true[:5].cpu().numpy()}")
        print(f"correct: {correct[:5].cpu().numpy()}")
        print(f"large_body: {large_body[:5].cpu().numpy()}")
        print(f"weights: {weights[:5].cpu().numpy()}")
        print(f"raw loss: {loss[:5].detach().cpu().numpy()}")
        print(f"weighted loss: {weighted_loss[:5].detach().cpu().numpy()}")
        print(f"Mean loss: {weighted_loss.mean().item()}")
