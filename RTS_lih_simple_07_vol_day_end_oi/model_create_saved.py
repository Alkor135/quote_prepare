...
import os
from RTS_lih_simple_07_vol_day_end_oi.data_read import data_load
from RTS_lih_simple_07_vol_day_end_oi.data_processing import balance_classes, calculate_pnl
...
class CandlestickDataset(Dataset):
    def __init__(self, X_candle, X_volume, X_day, X_dd, X_io, y):
        ...
        self.X_dd = torch.tensor(X_dd, dtype=torch.long)  # Дней до экспирации как long (категориальные)
...
for counter in range(1, 101):
    ...
    end_date = '2024-01-01'
    # Загрузка данных с фичами и таргетом
    ...
    X_candle, X_volume, X_day, X_dd, X_io, y = map(
        np.array, [X_candle, X_volume, X_day, X_dd, X_io, y]
        )

    # Разделение на train/test
    split = int(0.85 * len(y))
    X_train_candle, X_train_volume, X_train_day, X_train_dd, X_train_io, y_train = map(
        lambda x: x[:split], [X_candle, X_volume, X_day, X_dd, X_io, y]
        )
    X_test_candle, X_test_volume, X_test_day, X_test_dd, X_test_io, y_test = map(
        lambda x: x[split:], [X_candle, X_volume, X_day, X_dd, X_io, y]
        )
